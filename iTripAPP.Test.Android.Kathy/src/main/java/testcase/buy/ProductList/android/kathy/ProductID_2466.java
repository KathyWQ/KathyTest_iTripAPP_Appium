...
public class ProductID_2466 extends CommonClass_iTripAPP {
	/**
	 * 
	 * Buy Product_2466
	 * Author.Kathy
	...
	@Test
	public void BuyProduct2466()throws Exception{
		
		//点击首页搜索框，输入产品ID 2466
		getAppiumDriver().findElementsByClassName("android.widget.TextView").get(1).click();
		getAppiumDriver().findElementsByClassName("android.widget.TextView").get(0).sendKeys("2466");
		getAppiumDriver().findElementsByClassName("android.widget.TextView").get(1).click();	
		     	System.out.println('\n'+"点击搜索成功");
		     	 Thread.sleep(5000);
		...
		getAppiumDriver().findElementsByClassName("android.widget.TextView").get(2).click();
		        ...
		        //点击立即预定按钮
		getAppiumDriver().findElementById("com.aoliday.android.phone:id/order_now_view").click();
		        System.out.print('\n'+"点击立即预定成功");
		        Thread.sleep(5000);
		        //操作webview页面
		...
		getAppiumDriver().findElementByXPath("//android.view.View[contains(@content-desc,'30')]").click();
		       Thread.sleep(5000);
				        ...
				        Thread.sleep(5000);
			        // 填写旅客基本信息
		getAppiumDriver().findElementsByClassName("android.widget.EditText").get(0).sendKeys("爱去自由测试帐号");
		...
		getAppiumDriver().findElementsByClassName("android.widget.EditText").get(4).sendKeys("02890998478");
				        	...
				        	//旅客附加信息	
		getAppiumDriver().findElementsByXPath("//android.widget.EditText[contains(@content-desc,'请输入')]").get(0).sendKeys("HK12:30");
    	//屏幕宽度大小
...
getAppiumDriver().swipe(width / 2, height * 3 / 4, width / 2, height / 4, 3000);
		...
		getAppiumDriver().findElementsByXPath("//android.view.View/android.widget.EditText[contains(@content-desc,'请输入')]").get(0).sendKeys("CD12:30");
		getAppiumDriver().findElementsByXPath("//android.widget.EditText[contains(@content-desc,' ')]").get(0).sendKeys("2");				        	
